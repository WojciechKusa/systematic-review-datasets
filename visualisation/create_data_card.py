...
def gen_latex(dataset_name, helper, splits, schemas, fig_path):
    ...
    # langs = [l.value for l in helper.languages] # fixme
    langs = [l for l in helper.languages]
    languages = " ".join(langs)
    if type(helper.license) is dict:
        license = helper.license.value.name
    else:
        license = helper.license  # fixme
    tasks = [" ".join(t.lower().split("_")) for t in helper.tasks]  # fixme
    tasks = ", ".join(tasks)
...
def draw_figure(data_name, data_config_name, schema_type, split_name: str):
    ...
    fig_path = f"{figures_path}/{fig_name}"
...
if __name__ == "__main__":
    ...
    #     print(latex_bod)
    # NON LOCAL
    for data_name in conhelps.available_dataset_names:
